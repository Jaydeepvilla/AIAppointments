...
export default function LeadsPage() {
 ...
 <ScrollArea className="flex-1 p-space-4 space-y-space-4.5 bg-[hsl(var(--foreground)/0.002)]" horizontal={false}>
                   {!selectedLead ?
                   ...
                   {["New", "Qualified", "Hot", "Booked", "Escalated", "Closed"].map((stage) =>
                                    <Button key={stage} size="sm" variant={selectedLead.status === stage ? "default" : "outline"} className="text-caption" disabled={updatingStatusId !== null} onClick={() => handleUpdateStatus(selectedLead.id, stage)}>
                   ...
                   {selectedLead.answers.length === 0 ?
                                  ...
                                  <ScrollArea className="space-y-space-2 max-h-56 pr-space-1" horizontal={false}>
