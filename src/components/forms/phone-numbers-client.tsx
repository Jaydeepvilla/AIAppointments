...
export function PhoneNumbersClient({ initialNumbers }: { initialNumbers: any[] }) {
 ...
 <Button className="gap-space-2 cursor-pointer h-8.5 px-space-3.5 radius-lg transition-all font-semibold text-caption">
 ...
 {/* Number Results Grid */}
 <div className="border border-[hsl(var(--foreground)/0.06)] radius-lg p-space-1.5 max-h-40 overflow-y-auto space-y-space-1 bg-slate-50/20 dark:bg-transparent sidebar-scroll">
 ...
 variant="outline"
 size="sm"
 onClick={() => { setIsOpen(false); setStatusMessage(null); }}
 ...
 disabled={isPending}
 className="h-8.5 text-caption font-semibold cursor-pointer px-space-4"
 >
 {isPending ? (
 <>
 ...
 <div className="px-space-4.5 py-space-3 text-caption text-muted-foreground/80 flex flex-col gap-space-2 bg-[hsl(var(--foreground)/0.005)]">
 ...
 <button
 onClick={() => handleToggleRecording(num.id, num.isRecordingEnabled)}
 ...
 <div className="px-space-4.5 py-space-3.5 text-caption text-muted-foreground/60 flex items-center justify-between bg-background">
