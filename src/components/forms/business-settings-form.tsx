...
export function BusinessSettingsForm({ settings, connections = [], bookingRules }: BusinessSettingsFormProps) {
 ...
 {/* Pill Tab Bar Navigation */}
 <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-space-2 mb-space-4 w-full">
 <Button
 type="button"
 variant="ghost"
 onClick={() => { setActiveTab("hours"); setSaveSuccess(false); }}
 className={cn(
 "flex items-center justify-center gap-space-2 px-space-4 py-space-2 text-sm font-medium rounded-full transition-all cursor-pointer flex-1",
 activeTab ==="hours"
 ?"bg-primary text-primary-foreground hover:bg-primary/90 hover:text-primary-foreground"
 :"bg-background border border-[hsl(var(--foreground)/0.1)] text-muted-foreground/80 hover:text-foreground hover:bg-muted/50"
 )}
 >
 <Clock className={cn("h-4 w-4", activeTab !=="hours"&&"opacity-70")} /> Hours & Holidays
 </Button>
 <Button
 type="button"
 variant="ghost"
 onClick={() => { setActiveTab("notifications"); setSaveSuccess(false); }}
 className={cn(
 "flex items-center justify-center gap-space-2 px-space-4 py-space-2 text-sm font-medium rounded-full transition-all cursor-pointer flex-1",
 ...
 >
 <Bell className={cn("h-4 w-4", activeTab !=="notifications"&&"opacity-70")} /> Languages & Rules
 </Button>
 <Button
 type="button"
 variant="ghost"
 onClick={() => { setActiveTab("bookingRules"); setSaveSuccess(false); }}
 className={cn(
 "flex items-center justify-center gap-space-2 px-space-4 py-space-2 text-sm font-medium rounded-full transition-all cursor-pointer flex-1",
 ...
 >
 <Settings2 className={cn("h-4 w-4", activeTab !=="bookingRules"&&"opacity-70")} /> Booking Rules
 </Button>
 <Button
 type="button"
 variant="ghost"
 onClick={() => { setActiveTab("calendars"); setSaveSuccess(false); }}
 className={cn(
 "flex items-center justify-center gap-space-2 px-space-4 py-space-2 text-sm font-medium rounded-full transition-all cursor-pointer flex-1",
 activeTab ==="calendars"
 ?"bg-primary text-primary-foreground hover:bg-primary/90 hover:text-primary-foreground"
 ...
 >
 <CalendarDays className={cn("h-4 w-4", activeTab !=="calendars"&&"opacity-70")} /> Calendars
 </Button>
 </div>
 ...
 Define the weekly slots when your receptionist can verify appointment times.
 </p>
 </div>
 </div>

 <div className="p-space-5 space-y-space-3.5 bg-[hsl(var(--foreground)/0.002)]">
 {DAYS_OF_WEEK.map((day) => {
 const dayHours = hours[day] || { open:"09:00", close:"17:00", closed: false };
 return (
 <div
 key={day}
 className="grid grid-cols-1 sm:grid-cols-3 items-center gap-space-4 p-space-3 rounded-xl border border-[hsl(var(--foreground)/0.04)] bg-[hsl(var(--foreground)/0.005)] hover:border-[hsl(var(--foreground)/0.08)] transition-all duration-200"
 >
 <div className="flex items-center justify-start gap-space-2 w-full sm:w-auto shrink-0">
 {/* Custom Sliding Toggle for Closed status */}
 <button
 type="button"
 role="switch"
 ...
 className={cn(
 "relative inline-flex h-6 w-10 shrink-0 cursor-pointer items-center rounded-full transition-colors focus-visible:outline-none",
 !dayHours.closed ?"bg-primary":"bg-[hsl(var(--foreground)/0.12)]"
 )}
 >
 <span className={cn(
 "pointer-events-none block h-5 w-5 rounded-full bg-background ring-0 transition-transform duration-200",
 !dayHours.closed ?"translate-x-[18px]":"translate-x-0.5"
 )} />
 </button>
 <Label onClick={() => handleClosedToggle(day)} className="capitalize text-caption font-semibold text-foreground cursor-pointer select-none">
 {day}
 </Label>
 </div>

 <div className="flex items-center justify-center gap-space-2.5">
 <div className="relative">
 <Clock className="absolute left-space-2.5 top-space-2.5 h-3.5 w-3.5 text-muted-foreground/40 pointer-events-none z-10"/>
 <Input
 type="time"
 value={dayHours.open}
 onChange={(e) => handleHourChange(day,"open", e.target.value)}
 disabled={dayHours.closed}
 className="w-36 text-caption h-8.5 bg-background border-[hsl(var(--foreground)/0.08)] disabled:opacity-30 disabled:cursor-not-allowed pl-space-8 pr-space-2 focus-visible:ring-primary/20"
 />
 </div>
 <span className="text-caption text-muted-foreground/60 uppercase font-semibold tracking-wider">to</span>
 <div className="relative">
 <Clock className="absolute left-space-2.5 top-space-2.5 h-3.5 w-3.5 text-muted-foreground/40 pointer-events-none z-10"/>
 <Input
 type="time"
 ...
 <span
 className={cn(
 "inline-flex text-[10px] font-normal border px-space-2 py-space-0.5 rounded-full uppercase tracking-wider",
 !dayHours.closed
 ...
 <p className="text-caption text-muted-foreground/65 mt-space-0.5">
 Select calendar dates when the receptionist should reply that the business is closed.
 </p>
 </div>
 </div>

 <div className="p-space-5 space-y-space-4 bg-[hsl(var(--foreground)/0.002)]">
 <div className="flex gap-space-2 max-w-sm">
 ...
 <Button type="button"onClick={handleAddHoliday} className="h-8.5 text-caption font-semibold px-space-4 rounded-lg bg-primary text-white cursor-pointer">
 Add Date
 </Button>
 </div>
 ...
 <div
 key={date}
 className="flex items-center gap-space-1.5 border border-primary/15 bg-primary/5 text-primary text-caption font-semibold px-space-2.5 py-space-1 rounded-lg"
 >
 ...
 <Button type="submit"disabled={isSaving} className="h-9 text-caption font-semibold text-white cursor-pointer gap-space-1.5 rounded-lg px-space-5">
 {isSaving ? (
 <>
 ...
 isSelected ?"bg-primary border-primary text-primary-foreground":"border-[hsl(var(--foreground)/0.15)] bg-background"
 )}
 >
 ...
 className={cn(
 "h-4 w-4 rounded border flex items-center justify-center transition-colors shrink-0 mt-space-0.5",
 isChecked ?"bg-primary border-primary text-primary-foreground":"border-[hsl(var(--foreground)/0.15)] bg-background"
 )}
 >
 ...
 className={cn(
 "h-4 w-4 rounded border flex items-center justify-center transition-colors shrink-0 mt-space-0.5",
 rules.allowRescheduling ?"bg-primary border-primary text-primary-foreground":"border-[hsl(var(--foreground)/0.15)] bg-background"
 )}
 >
 {rules.allowRescheduling && <Check className="h-3 w-3 text-white stroke-[3px]"/>}
 </Button>
 <div className="space-y-space-0.5">
 <Label className="text-caption font-semibold text-foreground cursor-pointer leading-tight">
 Allow Rescheduling
 ...
 className={cn(
 "h-4 w-4 rounded border flex items-center justify-center transition-colors shrink-0 mt-space-0.5",
 rules.allowCancellation ?"bg-primary border-primary text-primary-foreground":"border-[hsl(var(--foreground)/0.15)] bg-background"
 )}
 >
 {rules.allowCancellation && <Check className="h-3 w-3 text-white stroke-[3px]"/>}
 </Button>
 <div className="space-y-space-0.5">
 <Label className="text-caption font-semibold text-foreground cursor-pointer leading-tight">
 Allow Cancellation
 ...
 {prov.name}
 {connection && (
 <span className="inline-flex text-caption font-semibold bg-emerald-500/8 border border-emerald-500/15 text-emerald-600 dark:text-emerald-400 px-space-1.5 py-space-0.5 rounded-full uppercase tracking-wider">
 Connected
 </span>
 )}
 </h4>
 ...
 <Button
 type="button"
 className="h-8 px-space-3.5 flex items-center justify-center text-caption font-semibold text-muted-foreground hover:text-foreground hover:bg-[hsl(var(--foreground)/0.04)] cursor-pointer transition-colors border-r border-[hsl(var(--foreground)/0.07)] gap-space-1"
 onClick={() => handleViewLogs(connection)}
 >
 <FileText className="h-3.5 w-3.5 text-muted-foreground/60"/> Logs
 </Button>
 <Button
 type="button"
 className="h-8 px-space-3.5 flex items-center justify-center text-caption font-semibold text-rose-500 hover:bg-rose-500/5 cursor-pointer transition-colors gap-space-1"
 onClick={() => handleDisconnect(connection.id)}
 >
 <Trash2 className="h-3.5 w-3.5 text-rose-500/60"/> Disconnect
 </Button>
 </div>
 ) : (
 <Button
 type="button"
 onClick={() => handleConnectClick(prov.slug)}
 ...
 <Button type="submit"disabled={isConnecting} className="h-9 text-caption font-semibold px-space-5 text-white cursor-pointer rounded-lg">
 {isConnecting ? (
 <>
 ...
 <div className="px-space-5 py-space-4 space-y-space-3.5 max-h-72 overflow-y-auto pr-space-1 sidebar-scroll bg-[hsl(var(--foreground)/0.002)]">
 {isLoadingLogs ? (
 ...
 <div
 key={log.id}
 className="p-space-3 rounded-lg border border-[hsl(var(--foreground)/0.04)] bg-background flex items-start justify-between gap-space-4 text-caption"
 >
 ...
 {log.details && (
 <pre className="text-caption text-muted-foreground/80 overflow-x-auto whitespace-pre-wrap max-w-md font-mono mt-space-1 p-space-2 rounded-md bg-[hsl(var(--foreground)/0.02)] border border-[hsl(var(--foreground)/0.04)]">
 ...
 <Button type="button"onClick={() => setShowLogsModal(false)} className="h-9 text-caption font-semibold px-space-4 cursor-pointer rounded-lg">
 Close
 </Button>
 </div>
 </DialogContent>
