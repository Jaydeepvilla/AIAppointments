...
export function AgencyTeamClient({
  ...
  const handleRemoveMember = async (memberId: string) => {
    if (!confirm("Are you sure you want to remove this staff member? They will lose access to the agency portal.")) return;
  ...
  const getRoleBadge = (role: string) => {
    switch (role) {
      case "owner":
        return (
          <span className="inline-flex items-center gap-space-1 px-space-2 py-space-1 radius-md text-caption  uppercase tracking-wider bg-destructive/10 text-error-500 border border-error-500/20">
      ...
      case "admin":
        return (
          <span className="inline-flex items-center gap-space-1 px-space-2 py-space-1 radius-md text-caption  uppercase tracking-wider bg-primary/10 text-primary border border-primary/20">
      ...
      default:
        return (
          <span className="inline-flex items-center px-space-2 py-space-1 radius-md text-caption  uppercase tracking-wider bg-muted text-muted-foreground border border-border/40">
            Staff
          </span>
        );
    }
            ...
            <Dialog open={isOpen} onOpenChange={setIsOpen}>
              ...
              <DialogContent className="max-w-md bg-card border-border/80">
                ...
                <DialogFooter>
                  ...
                  <Button onClick={handleSendInvite} disabled={isPending} className="bg-primary text-primary-foreground ">
              ...
              invitations.map((invite) => (
                <div key={invite.id} className="p-space-3 bg-background/50 radius-lg border border-border/40 space-y-space-2 text-caption">
                  <div className="flex justify-between items-start">
                    ...
                    <span className="text-caption uppercase  bg-warning-500/10 text-warning-500 border border-warning-500/20 px-space-2 rounded">
                      {invite.role}
                    </span>
                  </div>
