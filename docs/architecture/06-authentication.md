...
## Purpose
Defines the structural architecture, responsibilities, and flows for the AuthN within the Enterprise Multi-Tenant AI Receptionist SaaS platform.

## Architecture Diagram
```mermaid
graph TD;
  A[Client] --> B[AuthN Service];
  B --> C[(Database)];
```

## Responsibilities
- Handle all core operations for AuthN.
...
3. Payload routed to AuthN Controller.
...
sequenceDiagram
    ...
    participant S as AuthN Service
    participant D as DB
    U->>A: Initiate Action
    A->>S: Validate & Route
    S->>D: Execute Query
