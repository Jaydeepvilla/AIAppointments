...
## Purpose
Defines the structural architecture, responsibilities, and flows for the Comms within the Enterprise Multi-Tenant AI Receptionist SaaS platform.

## Architecture Diagram
```mermaid
graph TD;
  A[Client] --> B[Comms Service];
  B --> C[(Database)];
```

## Responsibilities
- Handle all core operations for Comms.
...
3. Payload routed to Comms Controller.
...
sequenceDiagram
    ...
    participant S as Comms Service
    participant D as DB
    U->>A: Initiate Action
    A->>S: Validate & Route
    S->>D: Execute Query
