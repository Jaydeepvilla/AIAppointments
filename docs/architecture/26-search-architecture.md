...
## Purpose
Defines the structural architecture, responsibilities, and flows for the Full Text Search within the Enterprise Multi-Tenant AI Receptionist SaaS platform.

## Architecture Diagram
```mermaid
graph TD;
  A[Client] --> B[Full Text Search Service];
  B --> C[(Database)];
```

## Responsibilities
- Handle all core operations for Full Text Search.
...
3. Payload routed to Full Text Search Controller.
...
sequenceDiagram
    ...
    participant S as Full Text Search Service
    participant D as DB
    U->>A: Initiate Action
    A->>S: Validate & Route
    S->>D: Execute Query
